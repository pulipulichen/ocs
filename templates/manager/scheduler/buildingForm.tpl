...
<ul class="nav nav-tabs">
	<li class="current active"><a href="{$buildingUrl}">{$pageTitle|translate}</a></li>
	{if $buildingId}
...
<p class="text-center"><input type="submit" value="{translate key="common.save"}" class="btn btn-primary" /> 
    {if not $buildingId}<input type="submit" name="createAnother" value="{translate key="manager.scheduler.building.saveAndCreateAnother"}" class="btn btn-default" /> {/if}
    <input type="button" value="{translate key="common.cancel"}" class="btn btn-default" onclick="document.location.href='{url op="buildings"}'" />
</p>
