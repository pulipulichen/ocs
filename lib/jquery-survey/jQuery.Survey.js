...
(function ($) {
  ...
  $.fn[PLUGIN_NS].defaults = {
    ...
    templates: {
      ...
      radio: '<div class="input form-group" {{compareHook element}}><label><span class="inputLabel{{#if element.required}} required{{/if}}">{{element.label}}{{#if element.required}} <span class="ast">*</span> {{/if}}<span class="colon">:</span></span></label>'+
             '{{#each element.options}}'+
               '<div class="radio"><label><input type="radio" name="{{../element.name}}" {{onChangeUpdateById ../element.id "_" value}} value="{{value}}"{{#if ../element.required}} required{{/if}}{{#dataExists ../element.name value}} checked{{/dataExists}} /> {{value}}</label></div>'+
             '{{/each}}</div>',
