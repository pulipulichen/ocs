...
(function ($) {
  ...
  Plugin.prototype._renderElements = function(page, elements){
    ...
    for(i=0; i<l; i++){
      ...
      element.id = pageId+'_elem_'+(element.name||(element.type+'_'+i)).replace(/[^\w\s]|_/g, "");
      if(element.name){
    ...
    templates: {
      ...
      radio: '<div class="input form-group" {{compareHook element}}><label style="padding-right: 0.5rem;"><span class="inputLabel{{#if element.required}} required{{/if}}">{{element.label}}{{#if element.required}} <span class="ast">*</span> {{/if}}<span class="colon">:</span></span></label>'+
             '{{#each element.options}}'+
               '<label class="radio-inline"><input type="radio" name="{{../element.name}}" {{onChangeUpdateById ../element.id "_" value}} value="{{value}}"{{#if ../element.required}} required{{/if}}{{#dataExists ../element.name value}} checked{{/dataExists}} /> {{value}}</label>'+
             '{{/each}}</div>',
