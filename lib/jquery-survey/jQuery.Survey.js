  ...
  Plugin.prototype._renderElements = function(page, elements){
    ...
    for(i=0; i<l; i++){
      ...
      if (typeof(element.name) === 'undefined' && typeof(element.label) === "string" ) {
        element.name = escape(element.label.replace(/ /g,''));
      }
    ...
    templates: {
      ...
      radio: '<div class="input" {{compareHook element}}><label><span class="inputLabel{{#if element.required}} required{{/if}}">{{element.label}}{{#if element.required}}<span class="ast">*</span>{{/if}}<span class="colon">:</span></span></label>'+
             '{{#each element.options}}'+
               '<label><input type="radio" name="{{../element.name}}" {{onChangeUpdateById ../element.id "_" value}} value="{{value}}"{{#if ../element.required}} required{{/if}}{{#dataExists ../element.name value}} checked{{/dataExists}} /> {{value}}</label>'+
             '{{/each}}</div>',
      ...
      submit: '<input class="button" {{compareHook element}} type="button" onclick="$.survey(this).submit(); return false;" {{onChangeUpdateById element.id}} value="{{element.caption}}" />',
