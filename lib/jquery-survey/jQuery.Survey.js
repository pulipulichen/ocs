...
(function ($) {
  ...
  Plugin.prototype._template = function(templateName){
    var self = this;
    var _templatesName = "templates";
    if (typeof(self.options.report) === "boolean" && self.options.report === true) {
        _templatesName = "reportTemplates";
    }
    return self.options[_templatesName][templateName];
  };
  ...
  Plugin.prototype._renderElements = function(page, elements){
    ...
    for(i=0; i<l; i++){
      ...
      if (typeof(element.name) === 'undefined' && typeof(element.label) === "string" ) {
        element.name = element.label.replace(/ /g,'');
      }
      element.id = pageId+'_elem_'+i+'_'+(element.name||(element.type)).replace(/[^\w\s]|_/g, "");
      //console.log(element);
      if(element.name){
  ...
  $.fn[PLUGIN_NS].defaults = {
          ...
          (function(survey){
            ...
            registerHelper('dataExists', function(key, value, options){
                ...
                var arr = getData(this.element, key)||[];
                      ...
                      //    console.log(arr);
                      //}
                return (arr === value)?options.fn(this):options.inverse(this);
            });
    ...
    templates: {
      ...
      radio: '<div class="input form-group" {{compareHook element}}><label style="padding-right: 0.5rem;"><span class="inputLabel{{#if element.required}} required{{/if}}">{{element.label}}{{#if element.required}} <span class="ast">*</span> {{/if}}<span class="colon">:</span></span></label>'+
             '{{#each element.options}}'+
                '<label class="radio-inline">'+
                '<input type="radio" name="{{../element.name}}" {{onChangeUpdateById ../element.id "_" value}} value="{{value}}"{{#if ../element.required}} required{{/if}}{{#dataExists ../element.name value}} checked{{/dataExists}} />'+
                //'{{#dataExists ../element.name value}}{{../element.value}}A{{/dataExists}}'+
                '{{../element.value}}'+
                ' {{value}}'+
                '</label>'+
             '{{/each}}</div>',
      checkbox: '{{#if element.options}}'+
                  ...
                  '{{#each element.options}}'+
                    ' <label><input type="checkbox" name="{{../element.name}}[]" {{onChangeUpdateById ../element.id "_" value}} value="{{value}}" {{#dataExists ../element.name value}}checked {{/dataExists}}{{#if ../element.required}} required{{/if}} /> {{value}} </label>'+
                  '{{/each}}</div>'+
                '{{else}}'+
      ...
      submit: '<input class="btn btn-primary" {{compareHook element}} type="button" onclick="$.survey(this).submit(); return false;" {{onChangeUpdateById element.id}} value="{{element.caption}}" />',
      nextPage: '<button class="btn btn-primary" {{compareHook element}} onclick="$.survey(this).nextPage({{element.force}}); return false;">{{element.caption}}</button>',
      prevPage: '<button class="btn btn-default" {{compareHook element}} onclick="$.survey(this).prevPage({{element.force}}); return false;">{{element.caption}}</button>',
      ...
      select: '<div class="input" {{compareHook element}}><label><span class="inputLabel{{#if element.required}} required{{/if}}">{{element.label}}{{#if element.required}} <span class="ast">*</span> {{/if}}<span class="colon">:</span></span>'+
            ' <select {{onChangeUpdateById element.id}} name="{{element.name}}"{{#if element.required}} required{{/if}}{{#ifElementData element.name}} value="{{elementData element.name}}"{{/ifElementData}} placeholder="{{element.placeholder}}">'+
             ...
             '{{/if}}'+
             '{{#each element.options}}'+
               '<option {{#dataIs ../element.name value}}selected {{/dataIs}}value="{{value}}">{{value}}</option>'+
             '{{/each}}'+
            '</select></label></div>',
    },
    reportTemplates: {
      loading: '<div class="loading">Loading...</div>',
      hidden: '{{#ifElementData element.name}}<input type="hidden" {{onChangeUpdateById element.id}} name="{{element.name}}" value="{{elementData element.name}}"/>{{/ifElementData}}',
      text: '{{#ifElementData element.name}}<div class="input" {{compareHook element}}><label><span class="inputLabel{{#if element.required}} required{{/if}}">{{element.label}}'+
            '<span class="colon">:</span></span> {{elementData element.name}}</label></div>{{/ifElementData}}',
      email: '{{#ifElementData element.name}}<div class="input" {{compareHook element}}><label><span class="inputLabel{{#if element.required}} required{{/if}}">{{element.label}}'+
            '<span class="colon">:</span></span> {{elementData element.name}}</label></div>{{/ifElementData}}',
      phone: '{{#ifElementData element.name}}<div class="input" {{compareHook element}}><label><span class="inputLabel{{#if element.required}} required{{/if}}">{{element.label}}'+
            '<span class="colon">:</span></span> {{elementData element.name}}</label></div>{{/ifElementData}}',
      page: '<div class="input" {{compareHook element}}>{{#if page.title}}<h1>{{page.title}}</h1>{{/if}}{{renderElements page.elements}}{{renderElements page.options}}</div>',
      templateError: '<div class="error"><div class="heading">Template Name: {{template.name}}</div>'+
                     '<div class="message">Error Message: {{err.message}}</div><div class="source">Template Source: {{template.source}}</div></div>',
      noTemplateError: '<div class="error"><div class="heading">Template Name: {{template.name}}</div>'+
                       '<div class="message">Error Message: No template exists for {{template.name}}</div></div>',
      radio: '{{#ifElementData element.name}}<div class="input form-group" {{compareHook element}}><label><span class="inputLabel{{#if element.required}} required{{/if}}">{{element.label}}<span class="colon">:</span></span></label> '+

             '{{#each element.options}}'+
               '{{#dataExists ../element.name value}}{{value}}'+
             '{{/dataExists}}{{/each}}</div>{{/ifElementData}}',
      checkbox: '{{#ifElementData element.name}}{{#if element.options}}'+
                  '<div class="input" {{compareHook element}}><label><span class="inputLabel{{#if element.required}} required{{/if}}">{{element.label}}<span class="colon">:</span></span></label> '+
                  '{{#each element.options}}'+
                    '{{#dataExists ../element.name value}}<label>{{value}}</label>{{/dataExists}}'+
                  '{{/each}}</div>'+
                '{{else}}'+
                  ' <div class="input" {{compareHook element}}> <label> <span class="inputLabel{{#if element.required}} required{{/if}}">{{element.label}}{{#if element.required}} <span class="ast">*</span> {{/if}} <input type="checkbox" name="{{element.name}}" {{onChangeUpdateById element.id}} value="{{element.key}}"{{#if element.required}} required{{/if}}{{#ifElementData element.name}} checked{{/ifElementData}} /></label></div>'+
                '{{/if}}{{/ifElementData}}',
      ...
      markdown: '{{markdown element.source}}',
      handlebars: '{{handlebars element.source this}}',
      select: '{{#ifElementData element.name}}<div class="input" {{compareHook element}}><span class="inputLabel{{#if element.required}} required{{/if}}">{{element.label}}<span class="colon">:</span></span>'+
            
             '{{#each element.options}}'+
               '{{#dataIs ../element.name value}}{{value}}{{/dataIs}}'+
             '{{/each}}'+
            '</div>{{/ifElementData}}',
    }
  };
...
$.fn.initSurvey = function(_report) {
    var _formEle = $(this);
    var _dataName = _formEle.attr("jquery-survey-data");
    var _errorMsg = null;
    if (_formEle.attr("error-message") !== "undefined") {
        _errorMsg = _formEle.attr("error-message");
    }
    
    var _form = _formEle.parents("form:first");
    var _dataEle;
    if (_form.length === 1) {
        _dataEle = _form.find('[name="' + _dataName + '"]');
    }
    else {
        _dataEle = $('[name="' + _dataName + '"]');
    }
    
    if (!_formEle.hasClass("debug")) {
        _formEle.hide();
        _dataEle.hide();
    }
    
    // 檢查form
    var _display = $("<div></div>");
    if (_formEle.parents("form:first").length === 0) {
        _formEle.wrap("<form />");
        _dataEle.insertAfter(_formEle);
    }
    
    
    var _survey = _formEle.val();
    _survey = $.trim(_survey);
    if (_survey === "") {
        return;
    }
    _survey = '{"pages":[{"elements":' + _survey + '}]}';
    _survey = _survey.replace(new RegExp( "\\n", "g" ),"");
    _survey = _survey.replace(new RegExp( " ", "g" ),"");
    _survey = _survey.replace(new RegExp( " ", "g" ),"");
    //_survey = _survey.replace(new RegExp( "-", "g" ),"_");
    //_survey = _survey.replace(new RegExp( "/", "g" ),"_");
    //console.log(_survey);
    _survey = JSON.parse(_survey);
    
    //console.log(_survey);
    
    var _data = _dataEle.val();
    if ($.trim(_data) === "") {
        _data = "{}";
    } 
    _data = JSON.parse(_data);
    
    _display.insertAfter(_formEle);
    ...
    _display.survey({
        ...
        init: function(target){
            var _validator = target.closest('form').validate({
              errorPlacement: function(error, element){
                ...
                if (_errorMsg !== null) {
                    error.find("label").html(_errorMsg);
                }
          ...
          afterChange: function () {
            ...
            var _changeCallback = function () {
                ...
                var _form = $("<form></form>").append(_display.clone());
                ...
                _dataEle.val(JSON.stringify(data));
                _form.remove();
            };
            _display.find("input").change(_changeCallback);
            _display.find("textarea").change(_changeCallback);
          },
          beforeChange: function(from, to, next){
            ...
            if (from === void 0) {
                ...
                var _v = target.closest('form').validate();
                var _result = target.closest('form').valid();
                if (_result === true) {
                    var survey = $.survey(target);
                    var data = survey.pageData();
                    //var surveyData = survey.data();
                    survey.updateData(data);
                    _display.val(JSON.stringify(data));
                    _display.parents("form:first").submit();
                }
            }
          }
    });
