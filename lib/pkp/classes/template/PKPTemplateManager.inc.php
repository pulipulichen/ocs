...
class PKPTemplateManager extends Smarty {
	...
	function &getManager($request = null) {
		...
		if ($instance === null) {
			$instance = new TemplateManager($request);
                ...
                AppLocale::requireComponents(array(LOCALE_COMPONENT_OCS_MANAGER, LOCALE_COMPONENT_OCS_DIRECTOR, LOCALE_COMPONENT_PKP_MANAGER)); // FIXME: For timeline constants
		return $instance;
	}
	...
	function smartyLoadUrlInDiv($params, &$smarty) {
		...
		if (isset($params['loadMessageId'])) {
			...
			$translatedLoadMessage = __($loadMessageId, $params);
		} else {
